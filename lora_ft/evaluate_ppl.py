...
def evaluate_ppl(dataset_name, model, tokenizer, ctx_length):
    ...
    if dataset_name == "wikitext":
        ...
        test = load_from_disk("../datasets/wikitext/test")
...
def eval_llm(model, tokenizer, task_list=["boolq","piqa","hellaswag","winogrande","arc_challenge","arc_easy","openbookqa"], num_fewshot=0):
    ...
    model_name="llama-7b-hf_wanda_csl_sparsity_0.7"
...
def main(args):
    ...
    # ppl = evaluate_ppl("wikitext", pruned_model, tokenizer, args.ctx_length)
    # print(f"pruned model perplexity on wikitext {ppl}")


    model = PeftModel.from_pretrained(pruned_model,args.lora_weights,torch_dtype=torch.float16)

    model.eval()

    ppl = evaluate_ppl("wikitext", model, tokenizer, args.ctx_length)
    print(f"finetune model perplexity on wikitext {ppl}")

    # # if args.eval_zero_shot:
    # # task_list_dict = {0: ["boolq", "rte","hellaswag","winogrande", "arc_easy","arc_challenge", "openbookqa"]}
    # task_list_dict = {0: [args.task]}
    # accelerate=False 
    # for num_shot in [0]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--model', type=str,default="../pruned/llama-7b-hf_wanda_csl_sparsity_0.7/"
    )
    parser.add_argument(
        '--cache_dir', type=str, default="../pruned"  
    )
    parser.add_argument(
        '--lora_weights', type=str, default="./lora_llama-7b-hf_wanda_csl_sparsity_0.7" 
    )
    parser.add_argument(
